...
namespace Íjász
{
    ...
    public sealed class Panel_Indulók : Control
    {
        ...
        public void Induló_EredményekNövelés(string _név)
        {
            if (!Program.database.Induló_EredményekNövelés(_név)) { MessageBox.Show("Adatbázis hiba!", "Hiba", MessageBoxButtons.OK, MessageBoxIcon.Error); return; }
        ...
        //Nincs invokeolva, csak belső hívásra!!
        public void Induló_EredményekCsökkentés(string _név)
        {
            if (!Program.database.Induló_EredményekCsökkentés(_név)) { MessageBox.Show("Adatbázis hiba!", "Hiba", MessageBoxButtons.OK, MessageBoxIcon.Error); return; }
        ...
        public void eredmények_hozzáadás(string _azonosító, Eredmény _eredmény)
        {
            Induló_EredményekNövelés(_eredmény.név);
        }

        public void eredmények_módosítás(string _azonosító, Eredmény _eredeti, Eredmény _eredmény)
        {
            if (_eredeti.név != _eredmény.név)
            {
                Induló_EredményekCsökkentés(_eredeti.név);
                Induló_EredményekNövelés(_eredmény.név);
            }
        }
        ...
        {
            Induló_EredményekCsökkentés(_eredmény.név);
        }
        private void beírás_Click(object sender, EventArgs e)
        {
             foreach (DataGridViewRow current in table.Rows)
            {
                try
                {
                    ...
                    {
                        eredmény_form = new Form_Induló_Teszt(current.Cells[0].Value.ToString());
        ...
        private void módosítás_Click(object _sender, EventArgs _event)
        {
           ...
           /* Form_Induló induló = new Form_Induló(new Induló(data.Rows[table.SelectedRows[0].Index][0].ToString(),
        ...
        void keresés_TextChanged(object sender, EventArgs e)
       {
        ...
        public sealed class Form_Induló_Teszt : Form
        {
            ...
            private void InitializeContent()
            {
                ...
                if (0 < versenyek.Count) combo_verseny.SelectedItem = combo_verseny.Items[0];

                else MessageBox.Show("Nincsen még egy verseny sem felvéve, először rögzítsen egyet!", "Hiba", MessageBoxButtons.OK, MessageBoxIcon.Error);
                ...
                if (0 < íjtípusok.Count) combo_íjtípus.SelectedText = íjtípusok[0].azonosító;
                else MessageBox.Show("Nincsen még egy íjtípus sem felvéve, először rögzítsen egyet!", "Hiba", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            {
                bool talalat = false;
                List<Eredmény> eredmények = Program.database.Eredmények(combo_verseny.SelectedItem.ToString());
                foreach (Eredmény item in eredmények)
                {
                    if (item.név == label_név.Text)
                    {
                        combo_íjtípus.SelectedItem = item.íjtípus;
                        combo_csapat.SelectedItem = item.csapat;
                        check_megjelent.Checked = (bool)item.megjelent;
                        talalat = true;
                        return;
                    }
                }
            ...
            private void InitializeData(Eredmény _eredmény)
            {
                ...
                //combo_csapat.SelectedIndex = _eredmény.csapat - 1;
            }
            ...
            void combo_verseny_SelectedIndexChanged(object sender, EventArgs e)
            {
                bool talalat = false;
                List<Eredmény> eredmények = Program.database.Eredmények(combo_verseny.SelectedItem.ToString());
                foreach (Eredmény item in eredmények)
            ...
            private void rendben_Click(object _sender, EventArgs _event)
            {
                ...
                lock (Program.datalock)
                {
                    ...
                    if (eredmény != null)
                    {
                        Eredmény módosított = new Eredmény(eredmény.Value.név, eredmény.Value.sorszám, combo_íjtípus.Text, Convert.ToInt32(combo_csapat.Text), eredmény.Value.találat_10,
                            eredmény.Value.találat_08, eredmény.Value.találat_05, eredmény.Value.mellé, eredmény.Value.összpont, eredmény.Value.százalék, check_megjelent.Checked);
                        Program.mainform.eredmények_panel.Eredmény_Módosítás(combo_verseny.Text, eredmény.Value, módosított);
                        ...
                        if (MessageBox.Show("Nyomtassak beírólapot ennek a versenyzőnek: " + label_név.Text + "?", "Nyomtatás", MessageBoxButtons.YesNo) == DialogResult.Yes)
                            Nyomtat.print(Nyomtat.nyomtat_beirlap(combo_verseny.SelectedItem.ToString(), módosított));
                    }
                    else
                    {
                        Program.mainform.eredmények_panel.Eredmény_Hozzáadás(combo_verseny.Text, new Eredmény(label_név.Text, 0, combo_íjtípus.Text, combo_csapat.SelectedIndex + 1, 0, 0, 0, 0, 0, 0, check_megjelent.Checked));

                        eredmény = Program.database.Eredmény(combo_verseny.Text, label_név.Text);
                        if (eredmény != null)
                        {
                            if (MessageBox.Show("Nyomtassak beírólapot ennek a versenyzőnek: " + label_név.Text + "?", "Nyomtatás", MessageBoxButtons.YesNo) == DialogResult.Yes)
                            {
        ...
        public sealed class Form_Csapatlista : Form
        {
            ...
            private void InitializeForm()
            {
                ...
                ClientSize = new System.Drawing.Size(788 - 64, 700);
                MinimumSize = ClientSize;
                StartPosition = FormStartPosition.CenterScreen;
                FormBorderStyle = System.Windows.Forms.FormBorderStyle.FixedToolWindow;
            }
            ...
            private DataTable CreateSource2()
            {
                ...
                foreach (Eredmény item in eredmények)
                {
                   if (item.csapat == seged && item.megjelent==true)
                    {
                        DataRow row = data.NewRow();
