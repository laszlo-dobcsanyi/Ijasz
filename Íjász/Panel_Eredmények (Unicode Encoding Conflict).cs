...
namespace Íjász
{
    public struct Eredmény
    {
        ...
        public int mellé;
        public Nullable<int> összpont;
        public Nullable<int> százalék;
        public Nullable<bool> megjelent;

        public Eredmény(string _név, Nullable<Int64> _sorszám, string _íjtípus, int _csapat, int _találat_10, int _találat_08, int _találat_05, int _mellé, Nullable<int> _összpont, Nullable<int> _százalék, Nullable<bool> _megjelent)
        {
            név = _név;
            sorszám = _sorszám;
            íjtípus = _íjtípus;
    ...
    public delegate void Eredmény_Hozzáadva(string _azonosító, Eredmény _eredmény);
    ...
    {
        public Form_Eredmény eredmény_form;

        public Eredmény_Hozzáadva eredmény_hozzáadva;
        ...
        public Panel_Eredmények()
        {
            ...
            eredmény_form = new Form_Eredmény("", -666);
        }
        ...
        public void Eredmény_Hozzáadás(string _azonosító, Eredmény _eredmény)
        {
            ...
            {
                if ((_eredmény.sorszám = Program.database.ÚjEredmény(_azonosító, _eredmény)) == null)
                {
                ...
                if (eredmény_hozzáadva != null) eredmény_hozzáadva(_azonosító, _eredmény);
        ...
        ///
        private delegate void Eredmény_Hozzáadás_Hálózat_Callback(string _azonosító, Eredmény _eredmény, Connection _connection);
        public void Eredmény_Hozzáadás_Hálózat(string _azonosító, Eredmény _eredmény, Connection _connection)
        {
            if (InvokeRequired)
            {
                Eredmény_Hozzáadás_Hálózat_Callback callback = new Eredmény_Hozzáadás_Hálózat_Callback(Eredmény_Hozzáadás_Hálózat);
                Invoke(callback, new object[] { _azonosító, _eredmény, _connection });
            }
            else
            {
                Nullable<Eredmény> eredmény = null;
                if ((eredmény = Program.database.ÚjEredmény_Ellenőrzött(_azonosító, _eredmény)) == null)
                {
                    //MessageBox.Show("Adatbázis hiba!\nLehet, hogy van már ilyen azonosító?", "Hiba", MessageBoxButtons.OK, MessageBoxIcon.Error); return;
                    _connection.Send(ServerCommand.ERROR, "Hiba az új eredmény hozzáadásakor!");
                    return;
                }
                ...
                {
                    DataRow row = data.NewRow();
                    row[0] = eredmény.Value.név;
                    row[1] = eredmény.Value.sorszám;
                    row[2] = eredmény.Value.íjtípus;
                    row[3] = eredmény.Value.csapat;
                    row[4] = eredmény.Value.találat_10;
                    row[5] = eredmény.Value.találat_08;
                    row[6] = eredmény.Value.találat_05;
                    row[7] = eredmény.Value.mellé;
                    row[8] = eredmény.Value.összpont;
                    row[9] = eredmény.Value.százalék;
                    row[10] = eredmény.Value.megjelent;
                    data.Rows.Add(row);
                }

                if (MessageBox.Show("Nyomtassak beírólapot ennek a segédből " + _azonosító + " versenyhez hozzáadott " + _eredmény.név + " versenyzőnek?", "Nyomtatás", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    Nyomtat.print(Nyomtat.nyomtat_beirlap(_azonosító, _eredmény));

                if (eredmény_hozzáadva != null) eredmény_hozzáadva(_azonosító, eredmény.Value);
            }
        }
        ...
        public void Eredmény_Módosítás_Hálózat(string _azonosító, Eredmény _eredeti, Eredmény _eredmény, Connection _connection)
        {
            ...
            {
                Nullable<Eredmény> eredmény = null;
                if ((eredmény = Program.database.EredményMódosítás_Ellenőrzött(_azonosító, _eredeti, _eredmény)) == null)
                {
                ...
                if (MessageBox.Show("Nyomtassak új beírólapot ennek a segédből " + _azonosító + " versenyen módosított " + _eredmény.név + " versenyzőnek?", "Nyomtatás", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    Nyomtat.print(Nyomtat.nyomtat_beirlap(_azonosító, _eredmény));
        ...
        private delegate void Eredmény_Beírás_Hálózat_Callback(string _név, string _verseny, string _íjtípus, int _csapat, bool _megjelent, Connection _connection);
        public void Eredmény_Beírás_Hálózat(string _név, string _verseny, string _íjtípus, int _csapat, bool _megjelent, Connection _connection)
        {
            if (InvokeRequired)
            {
                Eredmény_Beírás_Hálózat_Callback callback = new Eredmény_Beírás_Hálózat_Callback(Eredmény_Beírás_Hálózat);
                Invoke(callback, new object[] { _név, _verseny, _íjtípus, _csapat, _megjelent, _connection});
            }
            else
            {
                Eredmény? eredmény = Program.database.EredményBeírás_Ellenőrzött(_név, _verseny, _íjtípus, _csapat, _megjelent);
                if (eredmény == null)
                {
                ...
                {
                    if (MessageBox.Show("Nyomtassak beírólapot ennek a segédből " + _verseny + " versenyre beírt versenyzőnek: " + _név + "?", "Nyomtatás", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        Nyomtat.print(Nyomtat.nyomtat_beirlap(_verseny, eredmény.Value));
                    }
                }
            }
        }
        ...
        private void hozzáadás_Click(object _sender, EventArgs _event)
        {
            ...
            eredmény_form = new Form_Eredmény(combo_versenyek.Text, verseny_összpont);
        ...
        private void módosítás_Click(object _sender, EventArgs _event)
        {
           ...
           /* eredmény_form = new Form_Eredmény(combo_versenyek.Text, verseny_összpont, new Eredmény(data.Rows[table.SelectedRows[0].Index][0].ToString(),
                Convert.ToInt32(data.Rows[table.SelectedRows[0].Index][1]), data.Rows[table.SelectedRows[0].Index][2].ToString(),
                data.Rows[table.SelectedRows[0].Index][3].ToString(), Convert.ToInt32(data.Rows[table.SelectedRows[0].Index][4]),
                Convert.ToInt32(data.Rows[table.SelectedRows[0].Index][5]),
                Convert.ToInt32(data.Rows[table.SelectedRows[0].Index][6]), Convert.ToInt32(data.Rows[table.SelectedRows[0].Index][7]),
                Convert.ToInt32(data.Rows[table.SelectedRows[0].Index][8]), Convert.ToInt32(data.Rows[table.SelectedRows[0].Index][9])));
            */
            ...
            foreach (DataGridViewRow current in table.Rows)
            {
                ...
                {
                    eredmény_form = new Form_Eredmény(combo_versenyek.Text, verseny_összpont, new Eredmény(
        ...
        private void törlés_Click(object _sender, EventArgs _event)
        {
            ...
            {
               /* Eredmény_Törlés(combo_versenyek.Text, new Eredmény(data.Rows[table.SelectedRows[0].Index][0].ToString(),
                    Convert.ToInt32(data.Rows[table.SelectedRows[0].Index][1]), data.Rows[table.SelectedRows[0].Index][2].ToString(),
                    data.Rows[table.SelectedRows[0].Index][3].ToString(), Convert.ToInt32(data.Rows[table.SelectedRows[0].Index][4]),
                    Convert.ToInt32(data.Rows[table.SelectedRows[0].Index][5]),
                    Convert.ToInt32(data.Rows[table.SelectedRows[0].Index][6]), Convert.ToInt32(data.Rows[table.SelectedRows[0].Index][7]),
        ...
        private void lezárás_Click(object _sender, EventArgs _event)
        {
            ...
            lock (Program.datalock)
            {
                ...
                /*List<Korosztály> korosztályok = Program.database.Korosztályok(combo_versenyek.Text);
                for (int outer = 0; outer < korosztályok.Count; ++outer)
                {
                    for (int inner = outer + 1; inner < korosztályok.Count; ++inner)
                    {
                        if ((korosztályok[outer].alsó_határ <= korosztályok[inner].alsó_határ && korosztályok[inner].alsó_határ < korosztályok[outer].felső_határ)
                            || (korosztályok[outer].alsó_határ < korosztályok[inner].felső_határ && korosztályok[inner].felső_határ <= korosztályok[outer].felső_határ))
                            { MessageBox.Show("A verseny korosztályainál átfedés található: " + korosztályok[outer].azonosító + ", " + korosztályok[inner].azonosító + "!", "Hiba", MessageBoxButtons.OK, MessageBoxIcon.Error); return; }
                    }
                }*/
        ...
        public sealed class Form_Eredmény : Form
        {
            ...
            private Nullable<Eredmény> eredeti = null;
            private int összespont;

            private ComboBox combo_név;
            private Label label_sorszám;
            ...
            public Form_Eredmény(string _verseny, int _összespont)
            {
            ...
            {
                Text = "Eredmény";
                ClientSize = new System.Drawing.Size(464 -64, 250 + 64);
            ...
            private void InitializeContent()
            {
                ...
                Label sorszám = new Label();
                sorszám.Text = "Sorszám:";
                sorszám.Location = new System.Drawing.Point(név.Location.X, 16 + 1 * 32);
                ...
                csapat.Location = new System.Drawing.Point(név.Location.X, 16 + 0 * 32);

                Label találat_10 = new Label();
                találat_10.Text = "Tíz találat:";
                találat_10.Location = new System.Drawing.Point(név.Location.X, 16 + 1 * 32);

                Label találat_8 = new Label();
                találat_8.Text = "Nyolc találat:";
                találat_8.Location = new System.Drawing.Point(név.Location.X, 16 + 2 * 32);

                Label találat_5 = new Label();
                találat_5.Text = "Öt találat:";
                találat_5.Location = new System.Drawing.Point(név.Location.X, 16 + 3 * 32);

                Label mellé = new Label();
                mellé.Text = "Mellé találat:";
                mellé.Location = new System.Drawing.Point(név.Location.X, 16 + 4 * 32);

                Label összes = new Label();
                összes.Text = "Összes találat:";
                összes.Location = new System.Drawing.Point(név.Location.X, 16 + 5 * 32);

                Label százalék = new Label();
                százalék.Text = "Eredmény százalék:";
                százalék.Location = new System.Drawing.Point(név.Location.X, 16 + 6 * 32);
                százalék.AutoSize = true;

                Label megjelent = new Label();
                megjelent.Text = "Megjelent:";
                megjelent.Location = new System.Drawing.Point(név.Location.X, 16 + 7 * 32);
                ...
                label_sorszám = new Label();
                label_sorszám.Location = new System.Drawing.Point(sorszám.Location.X + sorszám.Size.Width + 16, sorszám.Location.Y);
            ...
            {
                combo_név.SelectedItem = null;
                label_sorszám.Text = "-";
                combo_íjtípus.SelectedItem = null;
                combo_csapat.SelectedItem = combo_csapat.Items[0];
                box_találat_10.Text = "0";
                box_találat_8.Text = "0";
                box_találat_5.Text = "0";
                box_mellé.Text = "0";
                label_összes.Text = "0";
                label_százalék.Text = "0%";
                box_megjelent.Checked = false;
            }
            ...
            private void InitializeData(Eredmény _eredmény)
            {
                ...
                label_sorszám.Text = _eredmény.sorszám.ToString();
                ...
                box_megjelent.Checked = _eredmény.megjelent.Value;
            }
            ...
            private void rendben_Click(object _sender, EventArgs _event)
            {
                ...
                lock (Program.datalock)
                {
                    if (eredeti != null)
                    {
                        Program.mainform.eredmények_panel.Eredmény_Módosítás(verseny_azonosító, eredeti.Value,
                            new Eredmény(combo_név.Text, Convert.ToInt64(label_sorszám.Text), combo_íjtípus.Text, combo_csapat.SelectedIndex + 1, Convert.ToInt32(box_találat_10.Text),
                    ...
                    {
                        Program.mainform.eredmények_panel.Eredmény_Hozzáadás(verseny_azonosító, new Eredmény(combo_név.Text, 0, combo_íjtípus.Text, combo_csapat.SelectedIndex + 1,
                            Convert.ToInt32(box_találat_10.Text), Convert.ToInt32(box_találat_8.Text), Convert.ToInt32(box_találat_5.Text), Convert.ToInt32(box_mellé.Text), összes, százalék, box_megjelent.Checked));
                    }
                }

                Close();
            }
            void eredmény_számolás(object sender, EventArgs e)
            {
                int találat_10;
